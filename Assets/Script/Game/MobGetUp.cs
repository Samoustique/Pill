...
public class MobGetUp : MonoBehaviour {
	...
	void Start (){
		...
		if (PhotonNetwork.isMasterClient) {
			view.RPC ("SetKinematic", PhotonTargets.AllBuffered, true);
		}
		...
		set{
			if (value){
				if (state == RagdollState.animated) {
					...
					//allow the ragdoll RigidBodies to react to the environment
					if (PhotonNetwork.isMasterClient) {
						view.RPC ("SetKinematic", PhotonTargets.AllBuffered, false);
					}
				...
				if (state==RagdollState.ragdolled) {
					...
					if (PhotonNetwork.isMasterClient) {
						if (anim.GetBoneTransform (HumanBodyBones.Hips).forward.y > 0) { //hip hips forward vector pointing upwards, initiate the get up from back animation
							view.RPC ("GetUpBack", PhotonTargets.AllBuffered);
						} else {
							view.RPC ("GetUpFront", PhotonTargets.AllBuffered);
						}
					}
				} //if (state==RagdollState.ragdolled)
			}	//if value==false	
	...
	void LateUpdate(){
		...
		if (state == RagdollState.blendToAnim) {
			if (!isFound && Time.time <= ragdollingEndTime + mecanimToGetUpTransitionTime) {
				...
				foreach (RaycastHit hit in hits) {
					if (!hit.transform.IsChildOf (transform)) {
					    newRootPosition.y = hit.point.y;//Mathf.Max(newRootPosition.y, hit.point.y); //
						isFound = true;
					}
				}
				
				if (PhotonNetwork.isMasterClient) {
					view.RPC ("ChangeTransformPosition", PhotonTargets.AllBuffered, newRootPosition);
				}
				...
				if (PhotonNetwork.isMasterClient) {
					view.RPC ("ChangeTransformRotation", PhotonTargets.AllBuffered, ragdolledDirection, animatedDirection);
				}
	...
	[PunRPC]
	private void ChangeTransformRotation(Vector3 ragdolledDirection, Vector3 animatedDirection){
		transform.rotation *= Quaternion.FromToRotation(animatedDirection.normalized, ragdolledDirection.normalized);
	}

	[PunRPC]
	private void ChangeTransformPosition(Vector3 newRootPosition){
		transform.position = newRootPosition;
	}

	[PunRPC]
	private void GetUpBack(){
		anim.SetTrigger ("getUpBack");
	}

	[PunRPC]
	private void GetUpFront(){
